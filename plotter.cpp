...
void Plotter::drawPlot(sf::RenderTarget &target, const sf::FloatRect &area,
                       ...
                       const std::vector<Series> &series,
                       const std::string &title) {
  // To verify if the vectors time and series are not empty and time has at
  // least two elements (tMax != tMin)
  assert(time.size() >= 2);
  assert(!series.empty());
  for (const auto &s : series) {
  ...
  for (const auto &s : series) {
    ...
    for (std::size_t i = 0; i < time.size(); ++i) {
      const float fx =
          area.left +
          area.width * static_cast<float>((time[i] - tMin) / (tMax - tMin));
      const float fy =
          area.top +
          area.height *
  ...
  drawLegend(target,
             sf::Vector2f(area.left + area.width - 160.f, area.top + 10.f),
             series);
}
...
void Plotter::show(const std::vector<double> &time,
  ...
  while (window_.isOpen()) {
    ...
    drawPlot(window_, bottomArea, time, hSeriesVec, "Integrale primo H(t)");
    window_.display();
  }
}
