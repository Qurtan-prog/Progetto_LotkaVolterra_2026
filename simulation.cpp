...
Parameters validated(Parameters const &params, double x0, double y0,
                     double dt) {
  if (x0 <= 0.0 || y0 <= 0.0 || params.A <= 0.0 || params.B <= 0.0 ||
      params.C <= 0.0 || params.D <= 0.0 || dt <= 0.0) {
    throw std::invalid_argument(
        "Simulation All parameters have to be positive");
  }
  return params;
}
Simulation::Simulation(Parameters const &params, double x0, double y0,
                       double dt)
    : params_(validated(params, x0, y0, dt)),
      dt_(dt),
      x_eq_(params.D / params.C),
      y_eq_(params.A / params.B) {
...
void Simulation::evolve() {
  ...
  if (!(x_new > 0.0) || !(y_new > 0.0)) {
    throw std::runtime_error(
        "Simulation::evolve: the simulation has diverged; next simulation try "
        "with a smaller dt !");
  }
...
std::size_t Simulation::size() const { return x_rel_.size(); }
...
Parameters const &Simulation::parameters() const { return params_; }

double Simulation::compute_H(double x_abs, double y_abs) const {
  return -params_.D * std::log(x_abs) + params_.C * x_abs + params_.B * y_abs -
         params_.A * std::log(y_abs);
}
